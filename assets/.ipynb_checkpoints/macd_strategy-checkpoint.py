...
def generate_and_plot_ema_signals(symbol, start_date, end_date):
    ...
    ema_signals_df["Delta"][short_window:] = np.where(
    (ema_signals_df["MACD"][short_window:] > ema_signals_df["Signal"][short_window:]), 1.0, 0.0)
    ...
    moving_avgs = ema_signals_df[["MACD", "Signal"]]

    # plt.figure(figsize=(10, 6))
    # plt.plot(ema_signals_df["Close"], label="Close")
    # plt.plot(moving_avgs, linewidth=1.5, label=["MACD", "Signal"])
    # plt.scatter(exit.index, exit, color="red", marker="v", s=50, label="Exit")
    # plt.scatter(entry.index, entry, color="purple", marker="^", s=50, label="Entry")
    # plt.xlabel("Index")
    # plt.ylabel("Price in $")
